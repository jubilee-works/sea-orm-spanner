...
pub async fn ensure_instance(path: &DatabasePath, config: &InstanceConfig) -> Result<bool, DbErr> {
    ...
    match result {
        Ok(mut op) => {
            op.wait(None)
                .await
                .map_err(|e| SpannerDbErr::Connection(format!("Instance creation failed: {}", e)))?;
            Ok(true)
        }
        Err(e) => {
...
pub async fn ensure_database(path: &DatabasePath, dialect: DatabaseDialect) -> Result<bool, DbErr> {
    ...
    match result {
        Ok(mut op) => {
            op.wait(None)
                .await
                .map_err(|e| SpannerDbErr::Connection(format!("Database creation failed: {}", e)))?;
            Ok(true)
        }
        Err(e) => {
            let err_str = e.to_string();
...
mod tests {
    ...
    fn test_database_path_parse() {
        ...
        assert_eq!(path.instance_path(), "projects/my-project/instances/my-instance");
        assert_eq!(path.full_path(), "projects/my-project/instances/my-instance/databases/my-db");
    }
